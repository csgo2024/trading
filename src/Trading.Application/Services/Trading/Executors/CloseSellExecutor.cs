...
public class CloseSellExecutor :
    INotificationHandler<KlineClosedEvent>
{
    private readonly ILogger<CloseSellExecutor> _logger;
    ...
    public async Task Handle(KlineClosedEvent notification, CancellationToken cancellationToken)
    {
        ...
        {
            if (strategy.AccountType == Domain.Entities.AccountType.Spot)
            {
                continue;
            }
    ...
    public async Task TryPlaceOrder(IAccountProcessor accountProcessor, Strategy strategy, CancellationToken ct)
    {
        ...
        while (currentRetry < maxRetries)
        {
            try
            {
                var orderResult = await accountProcessor.PlaceShortOrderAsync(
                    strategy.Symbol,
                    quantity,
                    price,
